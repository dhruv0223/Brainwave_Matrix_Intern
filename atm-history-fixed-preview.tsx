...
import { CreditCard, DollarSign, History, PiggyBank, Check, AlertCircle, X } from "lucide-react"

type Screen = "welcome" | "enter-account" | "enter-pin" | "main-menu" | "withdraw-amount" | "deposit-amount"
...
export default function ATMHistoryFixedPreview() {
  ...
  const addTransaction = (type: string, amount: number, description: string) => {
  ...
  const processInput = () => {
    ...
    switch (currentScreen) {
      ...
      case "withdraw-amount":
        ...
        if (withdrawAmount > 0 && withdrawAmount % 20 === 0) {
          if (currentAccount && currentAccount.balance >= withdrawAmount) {
            const newBalance = currentAccount.balance - withdrawAmount
            setCurrentAccount((prev) => (prev ? { ...prev, balance: newBalance } : null))
            addTransaction("WITHDRAWAL", withdrawAmount, "Cash withdrawal")
            showNotification(
              true,
      ...
      case "deposit-amount":
        ...
        if (depositAmount > 0) {
          if (currentAccount) {
            const newBalance = currentAccount.balance + depositAmount
            setCurrentAccount((prev) => (prev ? { ...prev, balance: newBalance } : null))
            addTransaction("DEPOSIT", depositAmount, "Cash deposit")
            showNotification(
              true,
              "Deposit Successful",
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-800 p-4">
      ...
      <div className="max-w-7xl mx-auto mb-4">
        <div className="bg-gray-800 text-white px-4 py-2 rounded-t-lg flex items-center justify-between">
          ...
          <span className="text-sm text-gray-300">v2.1.0 - HISTORY FIXED</span>
        </div>
      </div>
          ...
          <div className="w-80 p-6 space-y-6">
            ...
            {currentAccount && (
              <Card className="bg-green-50 border-green-300">
                <CardContent className="p-4">
                  <div className="text-center font-bold mb-2 text-sm text-green-800">Live Transaction Count</div>
                  <div className="text-center text-2xl font-bold text-green-600">
                    {currentAccount.transactions.length}
                  </div>
                  <div className="text-center text-xs text-green-700">Total Transactions</div>
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <strong>Account Holder:</strong> {currentAccount?.name}
                </div>
                <div>
                  <strong>Account Number:</strong> {currentAccount?.accountNumber}
                </div>
                <div>
                  <strong>Current Balance:</strong> ${currentAccount?.balance.toFixed(2)}
                </div>
                <div>
                  <strong>Total Transactions:</strong> {currentAccount?.transactions.length}
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded max-h-96 overflow-y-auto">
              {currentAccount?.transactions.length === 0 ? (
                <div className="text-center text-gray-500 py-8">
                  ...
                  <p className="text-sm">Start using your account to see history here.</p>
                </div>
              ) : (
                <div className="font-mono text-xs space-y-1">
                  <div className="font-bold border-b pb-2 mb-2">
                    TYPE | AMOUNT | BALANCE | DATE & TIME | DESCRIPTION
                  </div>
                  {currentAccount?.transactions.map((tx, idx) => (
