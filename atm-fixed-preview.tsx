...
export default function ATMFixedPreview() {
  const [currentScreen, setCurrentScreen] = useState<Screen>("welcome")
  const [inputBuffer, setInputBuffer] = useState("")
  const [tempAccountNumber, setTempAccountNumber] = useState("") // Fixed: Store account number separately
  ...
  const processInput = () => {
    ...
    setInputBuffer("") // Clear input buffer after getting the value

    switch (currentScreen) {
      case "enter-account":
        if (input.length >= 8) {
          setTempAccountNumber(input) // Store account number separately
          setCurrentScreen("enter-pin")
          console.log("Account number stored:", input) // Debug log
        } else {
      ...
      case "enter-pin":
        console.log("PIN entered:", input) // Debug log
        console.log("Account number:", tempAccountNumber) // Debug log

        if (input.length === 4) {
          // Use tempAccountNumber (stored separately) and input (PIN)
          const account = accounts.find((acc) => acc.accountNumber === tempAccountNumber && acc.pin === input)
          console.log("Authentication result:", account ? "Success" : "Failed") // Debug log
  ...
  const resetSession = () => {
    ...
    setInputBuffer("")
    setTempAccountNumber("") // Clear stored account number
  }
      ...
      case "enter-account":
        return (
          <div className="text-center text-white space-y-6">
            ...
            <div className="text-xs text-green-300">Debug: Current input = "{inputBuffer}"</div>
          </div>
        )
        ...
        return (
          <div className="text-center text-white space-y-6">
            ...
            <div className="text-xs text-green-300">
              Debug: Account = "{tempAccountNumber}", PIN length = {inputBuffer.length}
            </div>
          </div>
        )
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-800 p-4">
      ...
      <div className="max-w-7xl mx-auto mb-4">
        <div className="bg-gray-800 text-white px-4 py-2 rounded-t-lg flex items-center justify-between">
          ...
          <span className="text-sm text-gray-300">v2.1.0 - FIXED VERSION</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto bg-blue-600 rounded-b-lg shadow-2xl">
        <div className="flex">
          ...
          <div className="w-80 p-6 space-y-6">
            ...
            <Card className="bg-white">
              <CardContent className="p-4">
                <div className="text-center font-bold mb-2 text-sm">Test Accounts - FIXED!</div>
                <div className="text-xs space-y-1">
                  <div className="text-green-600 font-bold">✓ PIN Entry Now Works!</div>
                  <div>1234567890 / 1234 - John Doe</div>
                  <div>0987654321 / 5678 - Jane Smith</div>
                  <div>1111222233 / 9999 - Bob Johnson</div>
                  <div>4444555566 / 0000 - Alice Williams</div>
                  <div>7777888899 / 1111 - Michael Brown</div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-yellow-50 border-yellow-300">
              <CardContent className="p-4">
                <div className="text-center font-bold mb-2 text-sm text-yellow-800">Debug Info</div>
                <div className="text-xs space-y-1 text-yellow-700">
                  <div>Screen: {currentScreen}</div>
                  <div>Input: "{inputBuffer}"</div>
                  <div>Account: "{tempAccountNumber}"</div>
                  <div>User: {currentAccount?.name || "None"}</div>
                </div>
              </CardContent>
            </Card>
          </div>
